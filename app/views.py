...
def profile(request):
    ...
    return_josn = {"first_name": user_obj.first_name, "last_name": user_obj.last_name, "email": user_obj.email, "dept_name": d_name, "phone_no": str(profile_obj.phone_no), "address": profile_obj.address}
...
def sessions(request):
    ...
    if User.objects.get(pk = userid).is_staff is not True:
        ...
        for _i in course_enrolled_obj:
            ...
            dept_short_name = department.objects.get(pk = _i.dept_id_id).dept_short_name
            obj = {"course_id": course_obj.course_id, "course_string": dept_short_name.lower() + " " + course_obj.course_string, "session_id": session_obj.session_id,"session_name": session_obj.session_name , "course_name": course_obj.course_name, "term": _i.term, "year": _i.year, "url": "/"+dept_short_name.lower()+"/"+course_obj.course_string + "/session/"+  session_obj.session_name}
            print ("josn obj = ", obj)
            response.append(obj)
        ...
        for _i in session_obj:
            course_obj = course.objects.get(pk = _i.course_id_id)
            dept_short_name = department.objects.get(pk = _i.dept_id_id).dept_short_name
            obj = {"course_id": course_obj.course_id, "course_string": dept_short_name.lower() + " " + course_obj.course_string, "session_id": _i.session_id, "session_name": _i.session_name, "course_name": course_obj.course_name, "term": course_obj.term, "year": course_obj.year, "url": "/"+dept_short_name.lower()+"/"+course_obj.course_string + "/session/"+  _i.session_name}
            print ("josn obj = ", obj)
            response.append(obj)
...
def get_attendance(request, session_id, course_id):
    ...
    for _i in attendance_obj:
        ...
        month = _i.date.month
        day = _i.date.day
        obj = {"date": str(month)+"/"+str(day)+"/"+str(year), "type": 'attended' if _i.attendance else 'Not attended'}
        print ("obj = ", obj)
